...
def setup_logging(level: int, log_dir: Path) -> None:
    ...
    Logger secondaire (console uniquement, sans propagation) :
      - detect_ai_pipeline.console_summary
        → ligne compacte par média : nom + scores par modèle + final + prédiction
        → console uniquement, jamais dans les fichiers log
    """
    ...
    # Logger console_summary — console uniquement, sans propagation vers root
    # Utilisé par _log_file_detail pour la ligne compacte par média.
    summary_console = logging.StreamHandler()
    summary_console.setLevel(logging.INFO)
    summary_console.setFormatter(fmt_color)

    summary_logger = logging.getLogger("detect_ai_pipeline.console_summary")
    summary_logger.setLevel(logging.INFO)
    summary_logger.propagate = False   # ← ne remonte pas vers root → pas dans les fichiers
...
def _log_file_detail(
    ...
    Logique d'alerte divergence (v4 — modèles orthogonaux) :
      Avec 3 modèles orthogonaux, un désaccord inter-modèles est normal et
      attendu : chaque modèle détecte via un mécanisme différent, et une
      vraie détection peut reposer sur un seul modèle spécialisé.
      Une divergence élevée seule n'est donc PAS un signal d'alerte.

      Le WARNING n'est émis que si divergence élevée ET score final en zone
      grise [threshold_low, threshold_high] : c'est le seul cas où le
      désaccord rend le résultat réellement incertain (la décision finale
      pourrait basculer selon lequel des modèles est dans le vrai).
    """
    ...
    # Ligne compacte console uniquement — nom du média + scores inline + prédiction
    _summary = logging.getLogger("detect_ai_pipeline.console_summary")
    scores_inline = "  ".join(
        f"{n.split('/')[-1][:12]}={'N/A' if s is None else f'{s:.4f}'}"
        for n, s in scores.items()
    )
    _summary.info("📄 %-40s  %s  →  final=%.4f  [%s]",
                  filename, scores_inline, final_score, prediction.upper())

    in_grey_zone = threshold_low < final_score < threshold_high
    if divergence > divergence_threshold and in_grey_zone:
        logger.warning(
            "⚠  '%s' : score final en zone grise (%.4f, seuils=[%.2f, %.2f]) "
            "avec fort désaccord inter-modèles (écart-type=%.4f > seuil=%.4f) "
            "— vérification manuelle conseillée.",
            filename, final_score, threshold_low, threshold_high,
...
def analyze_image(
    ...
    _log_file_detail(path.name, scores, raw_scores, final_score, label,
                     div, divergence_threshold, threshold_low, threshold_high)
...
def _mongojob_worker_process(kwargs: dict) -> None:
    ...
    log_dir         = kwargs.pop("log_dir",   Path("."))
    ...
    _torch.set_grad_enabled(False)
    setup_logging(log_level, Path(log_dir))
    load_models(model_names, synthbuster_dir=synthbuster_dir)
    ...
    if args.mongojob:
        ...
        if workers <= 1:
            ...
            run_mongojob(db=db, worker_num=0, **{
                ...
                if k not in ("synthbuster_dir", "mongo_kwargs", "log_level", "log_dir")
            })
        else:
            ...
            try:
                ...
                # Les sémaphores multiprocessing (mode spawn) ne sont pas libérés
                # proprement après un SIGTERM — comportement inévitable, le warning
                # du resource_tracker est un faux positif sans conséquence.

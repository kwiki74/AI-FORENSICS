...
# Chaîne de résolution (du plus spécifique au plus général) :
#   1. SCHEMA/.env          — override local (non versionné, optionnel)
#   2. ~/AI-FORENSICS/.env  — source unique de vérité du projet
#   3. Variables d'environnement système (supervisord, systemd, shell)
# load_dotenv() est NON-écrasant : la première valeur trouvée a priorité.
try:
    from dotenv import load_dotenv
    _here   = Path(__file__).resolve().parent          # SCHEMA/
    _root   = _here.parent                             # ~/AI-FORENSICS/
    load_dotenv(_here   / ".env", override=False)      # 1. override local
    load_dotenv(_root   / ".env", override=False)      # 2. racine projet
except ImportError:
...
def patch_account_sync(neo4j: bool = False, elasticsearch: bool = False) -> dict:
    """Marque le document account comme synchronisé vers Neo4j / Elasticsearch."""
...
def create_indexes(db: Any) -> None:
    ...
    Idempotent : MongoDB ignore les index déjà existants à définition identique.

    ⚠ Exception : l'index hash_md5_1 sur media est supprimé et recréé
    automatiquement s'il existe sans partialFilterExpression (ancienne
    définition qui rejetait les documents avec hash_md5: null en doublon).
    ...
    # ------ correction index hash_md5_1 (migration) ------
    # Si l'index hash_md5_1 existe sans partialFilterExpression,
    # il bloque l'insertion de plusieurs médias sans hash (E11000 null).
    # On le supprime pour le recréer correctement ci-dessous.
    try:
        existing = {idx["name"]: idx for idx in db.media.list_indexes()}
        if "hash_md5_1" in existing:
            idx_def = existing["hash_md5_1"]
            if "partialFilterExpression" not in idx_def:
                db.media.drop_index("hash_md5_1")
                import logging as _log
                _log.getLogger(__name__).info(
                    "create_indexes: index hash_md5_1 recréé avec partialFilterExpression"
                )
    except Exception:
        pass  # collection media inexistante — premier lancement, pas de problème
    ...
    db.media.create_indexes([
        IndexModel([("hash_md5", ASCENDING)], unique=True, partialFilterExpression={"hash_md5": {"$type": "string"}}, name="hash_md5_1"),
